...
*************************************************** **/
	...
	var _scrollSpeed			= 1000,
		...
		// GOOGLE MAP
		$googlemap_latitude 	= 50.948620,
		$googlemap_longitude	= 6.944840,
		$googlemap_zoom			= 13,
...
/**	13. PORTFOLIO
*************************************************** **/
	function _galleryFilter() {
		jQuery("#portfolio ul.bstrap-tabs a").bind("click", function(e) {
			e.preventDefault();

			var _filter = jQuery(this).attr('href');
			var _filter = _filter.replace('#', '');

			if(_filter == "all") { // show all
				jQuery("#portfolio .item").animate({"width":window.portfolio_item_width + "px"}, 500, "easeInOutExpo");
			} else { // show by specific class only
				jQuery("#portfolio .item").each(function() {
					if(!jQuery(this).hasClass(_filter)) {
						jQuery(this).animate({"width":"0px"}, 500, "easeInOutExpo");
					} else {
						jQuery(this).animate({"width":window.portfolio_item_width + "px"}, 500, "easeInOutExpo");
					}
				});
			}
	...
	if(jQuery("#portfolio").length > 0) {
		jQuery(document).ready(function() {
			window.portfolio_item_width = jQuery("#portfolio .item").width();

			_galleryFilter(); 	// filter menu
...
*************************************************** **/
	...
	jQuery('#portfolio .item, #quick-blog .quick-hover').hover(function(){
